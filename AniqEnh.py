...
class AnimatedQuotesMod(loader.Module):
    ...
    strings = {
        "name": "AnimatedQuotes",
        "no_text": (
            "<emoji document_id=5312526098750252863>🚫</emoji> <b>Provide a text to create sticker with</b>"
        ),
        "processing": (
    ...
    strings_ru = {
        "no_text": (
            "<emoji document_id=5312526098750252863>🚫</emoji> <b>Укажи текст для создания стикера</b>"
        ),
        "processing": (
    ...
    strings_de = {
        "no_text": (
            "<emoji document_id=5312526098750252863>🚫</emoji> <b>Bitte gib einen Text an, um einen Sticker zu erstellen</b>"
        ),
        "processing": (
    ...
    strings_hi = {
        "no_text": (
            "<emoji document_id=5312526098750252863>🚫</emoji> <b>एक टेक्स्ट दें जिसके लिए एक स्टिकर बनाना है</b>"
        ),
        "processing": (
    ...
    strings_uz = {
        "no_text": (
            "<emoji document_id=5312526098750252863>🚫</emoji> <b>Sticker yaratish uchun matn kiriting</b>"
        ),
        "processing": (
    ...
    async def aniqcmd(self, message: Message):
        ...
        # Функция для получения текста из цепочки реплаев
        ...
        # Если текста нет в команде, ищем в реплаях
        if not args and message.is_reply:
            reply = await message.get_reply_message()
            args = await get_text_from_reply(reply)
        ...
        try:
            ...
            sent_msg = await processing_msg.respond(file=choice(query).document)
        except Exception as e:
            await utils.answer(processing_msg, str(e))
            return
