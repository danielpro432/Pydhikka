...
# meta name: MailGenFixed
# meta desc: Генератор временных email с безопасным чтением писем
...
@loader.tds
class MailGenFixed(loader.Module):
    """Генератор временных email с безопасным чтением писем"""
    ...
    strings = {
        ...
        "fetcherror": "❌ Не удалось получить письма (проверь соединение или API)"
    }
    ...
    def save_mails(self):
        try:
            ...
            print(f"[MailGenFixed] Ошибка сохранения: {e}")
    ...
    def load_mails(self):
        if os.path.exists(SAVE_FILE):
            try:
                ...
                print(f"[MailGenFixed] Ошибка загрузки: {e}")
                self.mails = []
    ...
    async def getmails(self, message):
        ...
        try:
            ...
            try:
                ...
                await utils.answer(message, self.strings["fetcherror"])
                return
