...
import asyncio
import logging
import re  # если ещё нет

def clean_text(text: str) -> str:
    """Заменяет неподдерживаемые Telegram inline теги на переносы строки."""
    text = re.sub(r'<br\s*/?>', '\n', text)  # <br> → перенос строки
    text = re.sub(r'<[^>]+>', '', text)      # все остальные HTML-теги убираем
    return text
...
answer = clean_text(answer)  # очищаем от HTML и <br>
