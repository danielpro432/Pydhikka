        ...
        try:
            with tempfile.TemporaryDirectory() as tmp:
                if has_photo:
                    ...
                    # Загружаем видео
                    file_path = os.path.join(tmp, "avatar_original.mp4")
                    await message.client.download_media(r.video, file_path)
                    
                    # Обрезаем видео до 6 секунд
                    output_path = os.path.join(tmp, "avatar.mp4")
    ...
    def _trim_video(self, input_path, output_path, duration=6):
        ...
        try:
            ...
            if file_size > 10 * 1024 * 1024:
                logger.error(f"Video too large after trimming: {file_size} bytes")
                return False
            
            logger.info(f"Video trimmed successfully: {file_size} bytes")
            return True
