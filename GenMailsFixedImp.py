...
class TempMailModule(loader.Module):
    ...
    strings = {
        ...
        "auto_update_set": "✅ Автообновление установлено на {}",
        "max_mails_set": "✅ Максимум почт установлен: {}",
    }
    ...
    async def _clean_deleted(self, uid):
        """Удаляем почты, которых нет у провайдера"""
        history = self._get_history(uid)
        new_history = []
        for rec in history:
            ...
            try:
                ...
                continue
        self._save_history(uid, new_history)
    ...
    async def usemail(self, message):
        """Выбрать активную почту (можно по номеру)"""
        ...
        # Проверка на число (выбор по номеру)
        if args[0].isdigit():
            idx = int(args[0]) - 1
            if 0 <= idx < len(history):
                email = history[idx].get("email")
        else:
            email = args[0]

        if not email:
            return await utils.answer(message, "❌ Почта не найдена")
        self.db.set(self.name, self._active_key(uid), email)
        await utils.answer(message, self.strings["set_active"].format(email))
    ...
    async def tinbox(self, message):
        ...
        try:
            ...
            await utils.answer(message, self.strings["inbox"].format(len(msgs), rec.get("email"), rec.get("provider"), out))
        except Exception as e:
            await utils.answer(message, f"⚠️ Ошибка API\nПровайдер: {rec['provider']}\nПричина: {str(e)[:RAW_LOG_LEN]}")

    # ---------------- Settings ----------------
    @loader.command()
    async def setautoupdate(self, message):
        """Включить/выключить автообновление: .setautoupdate on/off"""
        arg = utils.get_args_raw(message).lower()
        if arg not in ("on", "off"):
            return await utils.answer(message, "❌ Используй: on или off")
        self.auto_update = arg == "on"
        await utils.answer(message, self.strings["auto_update_set"].format(arg))

    @loader.command()
    async def setmaxmails(self, message):
        """Установить максимум почт (1-20): .setmaxmails 10"""
        arg = utils.get_args_raw(message)
        if not arg.isdigit() or not (1 <= int(arg) <= 20):
            return await utils.answer(message, "❌ Введи число от 1 до 20")
        self.max_mails = int(arg)
        await utils.answer(message, self.strings["max_mails_set"].format(arg))
