...
class MailTmProvider(BaseProvider):
    ...
    async def create_address(self):
        ...
        if stat not in (200, 201):
            # maybe account exists; try token
    ...
    async def list_messages(self, email, token):
        ...
        stat, ct, text = await self._get(f"{self.base}/messages",)
        # mail.tm often requires auth via session header; try with session get manually
    ...
    async def delete_account(self, email, token):
        # find account id then delete /me or /accounts/{id}
        headers = await self._auth_headers(token)
        async with aiohttp.ClientSession(headers={**HEADERS_BASE, **headers}) as s:
            # try /me
...
class MailsacProvider(BaseProvider):
    ...
    async def read_message(self, email, msg_id):
        ...
        stat, ct, text = await self._get(url)
        if stat != 200:
            raise RuntimeError(f"mailsac read HTTP {stat}")
        return {"body": text}
...
class TempMailModule(loader.Module):
    ...
    def _format_inbox_items(self, provider_name, raw_items):
        ...
        if provider_name == "mailtm":
            # raw_items is list of dicts with id, from, subject
            for r in raw_items:
    ...
    async def tread(self, message):
        ...
        for p in [provider] + [x for x in self.provider_order if x != provider]:
            ...
            try:
                if p == "mailtm":
                    token = meta.get("token")
                    msg = await prov_try.read_message(target_email, token, mid)
                    frm = msg.get("from")
    ...
    async def delmail(self, message):
        ...
        try:
            if provider == "mailtm":
                ...
                # providers often don't support account deletion; attempt message deletion for all messages then remove locally
                try:
                    msgs = []
                    if provider == "getnada":
